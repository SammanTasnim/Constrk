...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the construct_breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to construct_breadcrumb.
 * @return void
 */
function construct_breadcrumb( $args = array() ) {

  $breadcrumb = apply_filters( 'construct_breadcrumb_object', null, $args );

  if ( ! is_object( $breadcrumb ) ) {

    $breadcrumb = new construct_breadcrumb( $args );
  }
...
class construct_breadcrumb {
  ...
  /**
   * Sets up the breadcrumb trail properties.  Calls the `construct_breadcrumb::add_items()` method
   ...
   *     @type array     $labels         Text labels. @see construct_breadcrumb::set_labels()
   *     @type array     $post_taxonomy  Taxonomies to use for post types. @see construct_breadcrumb::set_post_taxonomy()
  ...
  public function __construct( $args = array() ) {
    ...
    $this->args = apply_filters( 'construct_breadcrumb_args', wp_parse_args( $args, $defaults ) );
  ...
  public function trail() {
    ...
    $breadcrumb = apply_filters( 'construct_breadcrumb', $breadcrumb, $this->args );
  ...
  protected function set_labels() {
    ...
    $this->labels = apply_filters( 'construct_breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
  }
  ...
  protected function set_post_taxonomy() {
    ...
    $this->post_taxonomy = apply_filters( 'construct_breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
  }
  ...
  protected function add_items() {
    ...
    $this->items = array_unique( apply_filters( 'construct_breadcrumb_items', $this->items, $this->args ) );
  }
  ...
  protected function map_rewrite_tags( $post_id, $path ) {
    ...
    if ( is_array( $matches ) ) {
      ...
      foreach ( $matches as $match ) {
        ...
        elseif ( '%day%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'proshoot' ) ) ) );
